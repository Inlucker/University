...
START_TEST(test_mysort_chars)
{
    int n = 26;
    char *mas = NULL;
    mas = calloc(n, sizeof (char));
    for (int i = 0; i < n; i++)
    {
        mas[i] = 'z' - i;
    }
    //mas = mas_int;
    //print_mas_char(n, mas);
...
START_TEST(test_mysort_doubles)
{
    int n = 20;
    double *mas = NULL;
    mas = calloc(n, sizeof (double));
    for (int i = 0; i < n; i++)
    {
        mas[i] = 11.36 - i;
    }
    //mas = mas_int;
    //print_mas_double(n, mas);

    mysort(mas, n, sizeof (double), (int(*)(const void *, const void *)) comp_double);
    ...
    for (int i = 0; i < n; i++)
    {
        if (fabs(mas[i] - (-7.64 + (double)i)) > EPS)
            areEqual = 0;
    }
...
START_TEST(test_key_input_zero_elems_after_key)
{
    ...
    int rez = key(mas, mas_end, mas_f, sr, m);
...
START_TEST(test_key_input_wrong_data)
{
    ...
    int rez = key(mas_end, mas, mas_f, sr, m);
